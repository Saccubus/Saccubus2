...
SimpleCommentLayer::SimpleCommentLayer(cinamo::Logger& log, const std::map<std::string, std::string> & config, layer::ThreadLayer* thread, bool isForked)
...
float SimpleCommentLayer::getX(float vpos, float screenWidth, std::shared_ptr<const Slot> slot)
{
...
void SimpleCommentLayer::queueComment(std::shared_ptr<item::Comment> comment)
{
...
void SimpleCommentLayer::draw(std::shared_ptr<saccubus::draw::Context> ctx, float vpos)
{
	{ /* 表示しないコメントを削除 */
...
bool SimpleCommentLayer::Slot::EndTimeComparator::operator() (const std::shared_ptr<const Slot>& a, const std::shared_ptr<const Slot>& b)
{
	return a->comment()->to() < b->comment()->to();
}
bool SimpleCommentLayer::Slot::EndTimeComparator::operator() (const float& a, const std::shared_ptr<const Slot>& b)
{
	return a < b->comment()->to();
}
bool SimpleCommentLayer::Slot::EndTimeComparator::operator() (const std::shared_ptr<const Slot>& a, const float& b)
{
